...
//    tiro = Toolkit.getDefaultToolkit().createImage(imagem);
  ...
  public static void inicializaAmbiente() {
    ...
    if(aux>=0) horScroll.setMaximum(aux+1);
    else horScroll.setVisible(false);
    aux=Codigos.linhas-25;
    if(aux>=0) verScroll.setMaximum(aux+1);
    else verScroll.setVisible(false);
    for(int i=0; i<40; i++) {
      for(int j=0; j<40; j++) {
...
//    atualizaAmostra(); }
  ...
  public void windowOpened(WindowEvent evt) {
    ambiente.repaint(); }
  public void windowClosing(WindowEvent evt) { fechaJanela(); }
